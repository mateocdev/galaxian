...
def create_starfield(world: esper.World) -> None:
    starfield_config = ServiceLocator.config_services.get(
        "assets/cfg/starfield.json")
    number_of_stars = starfield_config["number_of_stars"]
    speed = (starfield_config["vertical_speed"]["min"],
             starfield_config["vertical_speed"]["max"])
    blink = (starfield_config["blink_rate"]["min"],
    ...
    for _ in range(number_of_stars):
        ...
        color = pygame.Color(star_colors[color_choice]["r"],
                             star_colors[color_choice]["g"],
                             star_colors[color_choice]["b"])
        pos = pygame.Vector2(random.randint(0, ServiceLocator.window_services.get_width()),
                             random.randint(0, ServiceLocator.window_services.get_height()))
        vel = pygame.Vector2(0, random.randint(speed[0], speed[1]))
        blink_rate = random.randint(blink[0], blink[1])
        ...
        world.add_component(star_entity, CBlink(
