...
def create_player(world: esper.World):
    ...
    surface = ServiceLocator.images_services.get(
        player_config["image"])
...
def create_all_enemies(world: esper.World):
    enemies_config = ServiceLocator.config_services.get(
        "assets/cfg/enemies.json")
    level_config = ServiceLocator.config_services.get(
        "assets/cfg/level_01.json")
    fleet_entity = world.create_entity()
    world.add_component(fleet_entity, CEnemyFleet(
        1, level_config["enemy_speed"]["x"]))
    ...
    start_pos = pygame.Vector2(enemy_04_config["spawn_point"]["x"],
                               enemy_04_config["spawn_point"]["y"])
    ...
    for x in range(2):
        for y in range(1):
            ...
            create_enemy(world, pos, global_speed, score_value, score_value_attack, image, animations)_
...
def create_bullet_player(world: esper.World,
                         player: int) -> CBulletState:
    bullet_config = ServiceLocator.config_services.get(
        "assets/cfg/bullets.json")
    player_bullet_config = bullet_config["player_bullet"]
    size = pygame.Vector2(player_bullet_config["size"]["x"],
                          player_bullet_config["size"]["y"])
    vel = pygame.Vector2(0, 0)
    pos = pygame.Vector2(0, 0)
    color = pygame.Color(player_bullet_config["color"]["r"],
    ...
    speed = pygame.Vector2(player_bullet_config["speed"]["x"],
                           player_bullet_config["speed"]["y"])
...
def create_explosion(world: esper.World, pos: pygame.Vector2, explosion_type: str):
    explosion_config = ServiceLocator.config_services.get(
        "assets/cfg/explosions.json")
...
def create_bullet_enemy(world: esper.World,
    ...
    size = pygame.Vector2(enemy_bullet_config["size"]["w"],
                          enemy_bullet_config["size"]["h"])
    vel = pygame.Vector2(0, 0)
    color = pygame.Color(enemy_bullet_config["color"]["r"],
                         enemy_bullet_config["color"]["g"],
                         enemy_bullet_config["color"]["b"])
    bullet = create_square(world, pos, vel, size, color)
    speed = pygame.Vector2(enemy_bullet_config["velocity"]["x"] + vel_x,
