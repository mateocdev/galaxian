...
import pygame
import esper
from src.create import general_creator
...
def create_menu_interface(
        world: esper.World, use_prefab: bool) -> None:
    interface_config = ServiceLocator.config_services.get(
        "assets/cfg/interface.json")
    v_card_cfg = interface_config["vertical_card"]
    title_text_color = pygame.color.Color(interface_config["title_text_color"]["r"],
                                          interface_config["title_text_color"]["g"],
                                          interface_config["title_text_color"]["b"])
    title_text_size = interface_config["title_text_size"]

    # Create title with lives and score high score
    lives_txt = create_text(world, "LIVES", title_text_size, title_text_color,
                            pygame.Vector2(40, 20), TextAlignment.LEFT, False)
    high_score_txt = create_text(world, "HIGH SCORE", title_text_size,
                                 title_text_color, pygame.Vector2(80, 18), TextAlignment.LEFT, False)

    if use_prefab
    add_v_card_component(world, lives_txt, title_text_size,
                         v_card_cfg["v_speed"], v_card_cfg["v_offset"])
    add_v_card_component(world, high_score_txt, title_text_size,
                         v_card_cfg["v_speed"], v_card_cfg["v_offset"])


def create_interface_live(world: esper.World):
    player_config = ServiceLocator.config_services.get(
        "assets/cfg/player.json")
    lives = player_config["lives"]
    interface_config = ServiceLocator.config_services.get(
        "assets/cfg/interface.json")
    interface_lives_config = interface_config["lives"]
    lives_surface = ServiceLocator.images_services.get(
        interface_lives_config["image"])
...
def create_interface_counter_level(world: esper.World):
    interface_config = ServiceLocator.config_services.get(
        "assets/cfg/interface.json")
    text_counter_color = pygame.color.Color(interface_config["normal_text_color"]["r"],
    ...
    flag_pos = pygame.Vector2(interface_config["level_flag"]["pos"]["x"],
                              interface_config["level_flag"]["pos"]["y"])
    flag_vel = pygame.Vector2(0, 0)
    create_sprite(world, flag_pos, flag_vel, flag_surface)
    flag_txt_pos = pygame.Vector2(interface_config["level_flag"]["pos"]["x"],
                                  interface_config["level_flag"]["pos"]["y"])
    flag_txt_pos.x += 30
    flag_txt_pos.y += 5
...
def add_v_card_component(world: esper.World, entity: int, pos_y: float, speed: int, offset: int):
...
def create_pause_text(world: esper.World) -> tuple[CSurface, CBlink]:
    ...
    color = pygame.Color(interface_config["title_text_color"]["r"],
                         interface_config["title_text_color"]["g"],
                         interface_config["title_text_color"]["b"])
    pos = pygame.Vector2(interface_config["pause_text"]["pos"]["x"],
...
def create_win_text(world: esper.World) -> tuple[CSurface, CBlink]:
    interface_config = ServiceLocator.config_services.get(
        "assets/cfg/interface.json")
    color = pygame.Color(interface_config["normal_text_color"]["r"],
                         interface_config["normal_text_color"]["g"],
                         interface_config["normal_text_color"]["b"])
    pos = pygame.Vector2(interface_config["win_text"]["pos"]["x"],
    ...
    text_2 = create_text(world, get_text, size, color,
                         pos, TextAlignment.CENTER, False)
    return win, text_2
...
def create_game_start_text(world: esper.World) -> int:
    ...
    color = pygame.Color(interface_config["normal_text_color"]["r"],
                         interface_config["normal_text_color"]["g"],
                         interface_config["normal_text_color"]["b"])
    pos = pygame.Vector2(interface_config["game_start"]["pos"]["x"],
...
def create_game_over(world: esper.World):
    ...
    color = pygame.Color(interface_config["normal_text_color"]["r"],
                         interface_config["normal_text_color"]["g"],
                         interface_config["normal_text_color"]["b"])
    pos = pygame.Vector2(interface_config["game_over"]["pos"]["x"],
