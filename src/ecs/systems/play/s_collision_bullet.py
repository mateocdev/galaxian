...
def _check_enemy_collision(
    ...
    enemy_query = world.get_components(CTagEnemy, CEnemyState, CSurface, CTransform)
    for e_ent, (c_e_tag, c_e_st, c_e_s, c_e_t) in enemy_query:
        ...
        if (
            ...
            and c_b_st.state == BulletStates.FIRING
            and c_b_st.tag == "player"
        ):
            if c_e_st.state == EnemyStates.HIT:
                c_e_st.attack_channel.stop()
                c_lvl_mgr.current_attacks -= 1
                ServiceLocator.globals_service.player_score += (
                    c_e_tag.score_value_attack
                )
                if (
                    ServiceLocator.globals_service.player_score
...
def _check_player_collision(
    world: esper.World, bullet_rect: pygame.Rect, bullet_state: CBulletState, b_ent: int
):
    ...
    for _, (c_pl_st, c_pl_s, c_pl_t) in player_query:
        ...
        if (
            bullet_rect.colliderect(player_rect)
            and bullet_state.state == BulletStates.FIRING
            and bullet_state.tag == "enemy"
            and c_pl_st.state == PlayerStates.ALIVE
        ):
